...
//=============================================================================
 /*:
 * @plugindesc v1.01 The Gab Window functions as a window for random
...
Window_Gab.prototype.storeGabs = function() {
    $gameTemp._storedGabs = this._gabQueue.slice();
    if (this.contentsOpacity > 0) {
      $gameTemp._currentGab = this._currentGab.slice();
    } else {
      $gameTemp._currentGab = [];
    }
};

Window_Gab.prototype.restoreGabs = function() {
    if ($gameTemp._storedGabs) {
      this._gabQueue = $gameTemp._storedGabs;
      $gameTemp._storedGabs = undefined;
    }
    if ($gameTemp._currentGab && $gameTemp._currentGab.length > 0) {
      this._gabQueue.unshift($gameTemp._currentGab);
      $gameTemp._currentGab = undefined;
    }
};
...
SceneManager.push = function(sceneClass) {
    if (sceneClass !== Scene_Map && this._scene instanceof Scene_Map) {
