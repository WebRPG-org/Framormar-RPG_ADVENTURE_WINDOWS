...
@help
 ...
 ■ Changelog       ╒══════════════════════════╛
   ...
   v1.1 2015.11.19      > Fix bug when copying / transforming / moving event using other build
                        ...
                        to original map.
                        > Patch to be able to change region map from ANOTHER MAP...
                        (you can change the interior from outside your house for example)
                        (or using common event and call it anywhere)
...
Game_Interpreter.prototype.PC_updateOtherRegionMap = function(args) {
  var mapId = Number(args[0]);
  var region = Number(args[1]);
  var srcMapId = args[2].toUpperCase() === "NULL" ? null : Number(args[2]);
  if (isNaN(region)||isNaN(srcMapId)) return window.alert("wrong updateOtherRegionMap plugin command");
  this.updateOtherRegionMap(region, srcMapId);
};
...
_PluginCommands["UPDATE_OTHER_REGION_MAP"] = Game_Interpreter.prototype.PC_updateOtherRegionMap;
